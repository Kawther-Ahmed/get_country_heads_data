...
### Read the DB table in old_df dataframe
old_df = pd.read_sql('SELECT * FROM old_table', engine)


### Copy the new created dataframe df_sorted (from the above code) in new_df
new_df = df_sorted.copy()


### Identify records to delete (those in old_df but not in new_df)
records_to_delete = old_df.merge(new_df, how='outer', indicator=True).loc[lambda x: x['_merge'] == 'left_only']
records_to_delete = records_to_delete.drop(columns=['_merge'])


### Delete the records that are not in the new DataFrame -- using records_to_delete
# DB code


### Identify records to insert/update (those in new_df but not in old_df or have changed)
records_to_insert = pd.concat([old_df, new_df]).drop_duplicates(keep=False)


### Insert or update the changed/new records -- using records_to_insert
# DB code


### Print end line
print('Connecting and updating the DB part --> Done')